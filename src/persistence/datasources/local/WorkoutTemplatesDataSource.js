...
var fs = require('fs')
import { workoutTemplatesDbPath } from '../../../config/local-db-config'
...
class WorkoutTemplatesDao {
  constructor() {
    this.dbUtils = new DbUtils(this.getWorkoutTemplates)
  }

  getWorkoutTemplates = () => {
    // console.log(`getting workouts`)
    log.info(`getting workouts`)
    if (fs.existsSync(workoutTemplatesDbPath)) {
      let workoutTemplatesJson = fs.readFileSync(workoutTemplatesDbPath, 'utf8')
      let workoutTemplates = JSON.parse(workoutTemplatesJson)
      return workoutTemplates
    } else {
      throw new Error('workoutTemplates db failure.')
    }
  }
...
export default WorkoutTemplatesDao
